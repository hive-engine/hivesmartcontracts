...
describe('burndollar', function () {
  ...
  it('fails to covert token XXX to XXX.D', (done) => {
    ...
    let refBlockNumber =  74391382; 
     ...
     let res3 = await fixture.database.find({
      contract: 'burndollar',
      table: 'params',
      query: {},
     })
    ...
    assert.equal(JSON.parse(transactionsBlock1[36].logs).errors[0], 'not enough parent token to convert');
    assert.equal(JSON.parse(transactionsBlock1[37].logs).errors[0], 'token must be in pool with a stable coin')
    ...
    assert.equal(JSON.parse(transactionsBlock1[50].logs).errors[0], 'amount to convert must be >= 1')
      resolve();
